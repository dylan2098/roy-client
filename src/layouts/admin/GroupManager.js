...
import GroupUserContext from '../../context/GroupContext';

// core components
import Header from "components/Headers/Header.js";
import { useEffect, useReducer, useState } from 'react';
import reducer from '../../reducer/reduce';
import ListGroup from "components/Manager/Group/ListGroup";
import Paging from "components/Common/Paging";
import ModalForm from "components/Common/ModalForm";
import Services from "../../services/index";
import { useForm } from 'react-hook-form';
import groupConfig from '../../settings/groupConfig.json'
...
const GroupIndex = () => {
    ...
    const onReload = async () => {
        const res = await Services.getInstanceGroup().getAllGroup(search, Instance.getInstanceHelper().getLimitInTable(), indexPage);
    ...
    const onSubmitFormAdd = async (data) => {
        ...
        try {
            const res = await Services.getInstanceGroup().createGroup(data);
            if (res.statusCode !== 200) {
    ...
    const onSubmitFormUpdate = async (data) => {
        ...
        try {
            const res = await Services.getInstanceGroup().updateGroup(data);
            if (res.statusCode !== 200) {
    ...
    return (
        <>
            <GroupUserContext.Provider value={{ store, dispatch }}>
                <Header />
                {/* Page content */}
                                ...
                                <Row>
                                    ...
                                    <div className="col-6">
                                        <div className="form-inline mr-3 ml-3 mt-3 mb-3 float-right">
                                            <Button className="text-right" color="primary" type="button" onClick={() => toggleModelFormAdd()} > + {Language.getContent('ADD_GROUP', 'GROUP')} </Button >
                                        </div>
                                    </div>
                                </Row>

                                <ModalForm toggleModel={toggleModelFormAdd} open={openFormAdd} onSubmit={onSubmitFormAdd} dataMessage={dataMessageFormAdd} loading={loadingModelFormAdd} form={groupConfig["FORM_ADD"]} page={groupConfig["NAME"]} />
                                <ModalForm toggleModel={toggleModelFormUpdate} open={openFormUpdate} onSubmit={onSubmitFormUpdate} dataMessage={dataMessageFormUpdate} loading={loadingModelFormUpdate} form={groupConfig["FORM_UPDATE"]} item={itemUpdate} page={groupConfig["NAME"]} />

                                <ListGroup indexPage={indexPage} search={search} loading={loading} onClickFormUpdate={onClickFormUpdate} />
                                {
