...
import ListUser from "components/Manager/Users/ListUser";
import Paging from "components/Common/Paging";
import ModalForm from "components/Common/ModalForm";
import Services from "../../services/index";
import { useForm } from 'react-hook-form';
import userConfig from '../../settings/userConfig.json'
...
const UserIndex = () => {
    ...
    const onReload = async () => {
        const res = await Services.getInstanceUser().getAllUsers(search, Instance.getInstanceHelper().getLimitInTable(), indexPage);
                    ...
                    <Row>
                        <div className="col">
                            <Card className="shadow">
                                <Row>
                                    ...
                                    <div className="col-6">
                                        <div className="form-inline mr-3 ml-3 mt-3 mb-3 float-right">
                                            <Button className="text-right" color="primary" type="button" onClick={() => toggleModelFormAdd()} > + {Language.getContent("ADD_USER", "USER")} </Button >
                                        </div>
                                    </div>
                                </Row>
                                ...
                                <ModalForm toggleModel={toggleModelFormAdd} open={openFormAdd} onSubmit={onSubmitFormAdd} dataMessage={dataMessageFormAdd} loading={loadingModelFormAdd} form={userConfig["FORM_ADD"]} page={userConfig["NAME"]} />
                                <ModalForm toggleModel={toggleModelFormUpdate} open={openFormUpdate} onSubmit={onSubmitFormUpdate} dataMessage={dataMessageFormUpdate} loading={loadingModelFormUpdate} form={userConfig["FORM_UPDATE"]} item={itemUpdate} page={userConfig["NAME"]} />

                                <ListUser indexPage={indexPage} search={search} loading={loading} onClickFormUpdate={onClickFormUpdate} />
                                {
