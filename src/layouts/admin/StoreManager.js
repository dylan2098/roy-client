...
import ListStore from "components/Manager/Stores/ListStore";
...
const StoreIndex = () => {
    ...
    const onSubmitFormAdd = async (data) => {
        ...
        try {
            const res = await Services.getInstanceStore().createStore(data);
            if (res.statusCode !== 200) {
    ...
    const onSubmitFormUpdate = async (data) => {
        ...
        try {
            const res = await Services.getInstanceStore().updateStore(data);
            if (res.statusCode !== 200) {
                                ...
                                <Row>
                                    ...
                                    <div className="col-6">
                                        <div className="form-inline mr-3 ml-3 mt-3 mb-3 float-right">
                                            <Button className="text-right" color="primary" type="button" onClick={() => toggleModelFormAdd()} > + {Language.getContent('ADD_STORE', 'STORE')} </Button >
                                        </div>
                                    </div>
                                </Row>

                                <ModalForm toggleModel={toggleModelFormAdd} open={openFormAdd} onSubmit={onSubmitFormAdd} dataMessage={dataMessageFormAdd} loading={loadingModelFormAdd} form={StoreConfig["FORM_ADD"]} page={StoreConfig["NAME"]} />
                                <ModalForm toggleModel={toggleModelFormUpdate} open={openFormUpdate} onSubmit={onSubmitFormUpdate} dataMessage={dataMessageFormUpdate} loading={loadingModelFormUpdate} form={StoreConfig["FORM_UPDATE"]} item={itemUpdate} page={StoreConfig["NAME"]} />

                                <ListStore indexPage={indexPage} search={search} loading={loading} onClickFormUpdate={onClickFormUpdate} />
                                {
