...
import ListCatalog from "components/Manager/Catalog/ListCatalog";
...
const CatalogIndex = () => {
    ...
    const onSubmitFormAdd = async (data) => {
        ...
        try {
            const res = await Services.getInstanceCatalog().createCatalog(data);
            if (res.statusCode !== 200) {
    ...
    const onSubmitFormUpdate = async (data) => {
        ...
        try {
            const res = await Services.getInstanceCatalog().updateCatalog(data);
            if (res.statusCode !== 200) {
    ...
    return (
        <>
            <CatalogContext.Provider value={{ store, dispatch }}>
                <Header />
                {/* Page content */}
                                ...
                                <Row>
                                    ...
                                    <div className="col-6">
                                        <div className="form-inline mr-3 ml-3 mt-3 mb-3 float-right">
                                            <Button className="text-right" color="primary" type="button" onClick={() => toggleModelFormAdd()} > + {Language.getContent("ADD_CATALOG", "CATALOG")} </Button >
                                        </div>
                                    </div>
                                </Row>

                                <ModalForm toggleModel={toggleModelFormAdd} open={openFormAdd} onSubmit={onSubmitFormAdd} dataMessage={dataMessageFormAdd} loading={loadingModelFormAdd} form={catalogConfig["FORM_ADD"]} page={catalogConfig["NAME"]} />
                                <ModalForm toggleModel={toggleModelFormUpdate} open={openFormUpdate} onSubmit={onSubmitFormUpdate} dataMessage={dataMessageFormUpdate} loading={loadingModelFormUpdate} form={catalogConfig["FORM_UPDATE"]} item={itemUpdate} page={catalogConfig["NAME"]} />

                                <ListCatalog indexPage={indexPage} search={search} loading={loading} onClickFormUpdate={onClickFormUpdate} />
                                {
