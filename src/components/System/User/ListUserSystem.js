...
const ListUserSystem = (props) => {
    const Helper = Instance.getInstanceHelper();
    const Language = Instance.getInstaceLanguage();

    const { store, dispatch } = useContext(UserSystemContext);
    const [loading, setLoading] = useState(false);
    const [positionUser, setPositionUser] = useState();

    const listStatus = UserSystemConfig["STATUS"];
    const attributeDisplay = UserSystemConfig["ATTRIBUTE_DISPLAY"];
