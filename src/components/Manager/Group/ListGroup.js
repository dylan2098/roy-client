...
const ListGroup = (props) => {
    ...
    const { store, dispatch } = useContext(GroupUserContext);
    const [loading, setLoading] = useState(false);
    const [position, setPosition] = useState();

    const listStatus = groupConfig["STATUS"];
    const attributeDisplay = groupConfig["ATTRIBUTE_DISPLAY"];

    const listGroup = async (indexPage, search) => {
    ...
    const changeStatus = async (id, status, currentStatus) => {
        if (status !== currentStatus) {
            ...
            const res = await Services.getInstanceGroup().changeStatus(id, status);
    ...
    useEffect(() => {
        listGroup(props.indexPage, props.search);
