...
const ListUser = (props) => {
    ...
    const LIMIT = Helper.getLimitInTable();

    const listStatus = UserConfig["STATUS"];
    const attributeDisplay = UserConfig["ATTRIBUTE_DISPLAY"];

    const listUsers = async (indexPage, search) => {
        const res = await Services.getInstanceUser().getAllUsers(search, LIMIT, indexPage);
    ...
    const changeStatus = async (id, status, currentStatus) => {
        if (status !== currentStatus) {
            ...
            const res = await Services.getInstanceUser().changeStatus(id, status);
    ...
    const elementStatus = (loading, item, positionUser) => {
        ...
        if (loading && positionUser == item.id) {
    ...
    useEffect(() => {
        listUsers(props.indexPage, props.search);
                        ...
                        <tr>
                            {
                                attributeDisplay && attributeDisplay.map((attribute, index) => {
                                    return <th scope="col" className="text-center" key={index}>{Language.getContent(attribute, "USER")}</th>
                                })
                            }
                            <th scope="col" />
                        </tr>
                            ...
                            store.items.map((item, index) => {
                                return (
                                    <tr key={item.id + 1}>
                                        ...
                                        <td className="text-center">{item.role != 1 ? 'User' : 'Admin'}</td>
                                        <td className="text-center">
                                            <UncontrolledDropdown>
                                                ...
                                                >
                                                    {
                                                        elementStatus(loading, item, positionUser)
                                                    }
                                                </DropdownToggle>
