...
const ListPricebook = (props) => {
    const Language = Instance.getInstaceLanguage();

    const { store, dispatch } = useContext(PricebookContext);
    const [loading, setLoading] = useState(false);
    const [position, setPosition] = useState();

    const listStatus = pricebookConfig["STATUS"];
    const attributeDisplay = pricebookConfig["ATTRIBUTE_DISPLAY"];

    const listPricebook = async (indexPage, search) => {
    ...
    const changeStatus = async (id, status, currentStatus) => {
        if (status !== currentStatus) {
            ...
            const res = await Services.getInstancePricebook().changeStatus(id, status);
    ...
    useEffect(() => {
        listPricebook(props.indexPage, props.search);
                        ...
                        <tr>
                            {
                                attributeDisplay && attributeDisplay.map((attribute, index) => {
                                    return <th scope="col" className="text-center" key={index}>{Language.getContent(attribute, props.page)}</th>
                                })
                            }
                            <th scope="col" />
                        </tr>
